...
export const putDb = async (content) => {
  ...
  const dbOpen = await openDB(DB_NAME, 1)

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  // const YOUR_TX_VAR = db.transaction(DB_NAME, 'readwrite');
  const tx = dbOpen.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  // const YOUR_STORE_VAR = dbTransaction.objectStore(DB_NAME);
...
export const getDb = async () => {
  ...
  const dbOpen = await openDB(DB_NAME, 1)
  const tx = dbOpen.transaction(DB_NAME, 'readonly');
  const store = tx.objectStore(DB_NAME);
